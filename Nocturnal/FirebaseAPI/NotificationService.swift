...
struct NotificationService {
    ...
    func updatePermissionNotification(for uid: String, isPermitted: Bool, completion: FirestoreCompletion) {
        collection_notification.document(uid).collection("user-notification").getDocuments { snapshot, error in
    ...
    func postAceeptedNotification(to applicantUid: String, notification: Notification, completion: FirestoreCompletion) {
        ...
        updatePermissionNotification(for: uid, isPermitted: notification.isRequestPermitted) { error in
            ...
            postNotification(to: applicantUid, notification: notification) { error in
                ...
                print("successful posting notification")
                // also need to add nitification.applicantId to event.participants array
                EventService.shared.updateEvent(hostId: uid, acceptedApplicantId: applicantUid) { error in
                    guard error == nil else {
    ...
    func postDeniedNotification(to applicantUid: String, notification: Notification, completion: FirestoreCompletion) {
        ...
        updatePermissionNotification(for: uid, isPermitted: notification.isRequestPermitted) { error in
            ...
            postNotification(to: applicantUid, notification: notification) { error in
                ...
                EventService.shared.updateEvent(hostId: uid, deniedApplicantId: applicantUid) { error in
                    guard error == nil else {
